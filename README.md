...
后端会从 `backend/.env` 读取配置。仓库中已提供一个可提交的、去敏后的示例配置文件，你可以直接在此基础上修改本地值。
...
如果你只想验证基础下载链路，可先不配置真实的大模型密钥；如果要使用视频总结 / 问答功能，请将 `DEEPSEEK_API_KEY` 替换为你自己的有效值。
...
仓库中包含一个去敏后的 `backend/.env` 示例文件，便于本地快速启动。

请注意：

- 示例文件中的密钥占位值不能直接用于生产或真实调用
- 如果你要启用总结 / 问答能力，需要自行替换成真实可用的 API Key
- 不要把你自己的真实敏感密钥再次提交到公开仓库
